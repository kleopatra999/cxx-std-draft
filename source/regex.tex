...
\begin{itemdescr}
\pnum\returns  \tcode{rhs.compare(basic_string_ref<typename iterator_traits<BiIter>::value_type>(lhs.data(), lhs.size())) == 0}.\footnote{This and the other comparison operators ignore the \tcode{basic_string_ref}'s traits.}
\end{itemdescr}
...
\begin{itemdescr}
\pnum\returns  \tcode{rhs.compare(basic_string_ref<typename iterator_traits<BiIter>::value_type>(lhs.data(), lhs.size())) > 0}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum\returns  \tcode{lhs.compare(basic_string_ref<typename iterator_traits<BiIter>::value_type>(rhs.data(), rhs.size())) == 0}.
\end{itemdescr}
...
\pnum
\effects
Constructs an empty string \tcode{result} of type \tcode{basic_string<char_type, ST>} and
...
for the first and second forms of the function and
\tcode{m.format(out,} \tcode{fmt, fmt + char_traits<charT>::length(fmt), flags)} for the third. Finally, if such a match
...
type \tcode{basic_string<charT, ST>} and calls 
