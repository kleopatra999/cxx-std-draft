...
namespace std {
  ...
  // \ednote{It would be nice to remove the \tcode{string} overloads, but that could break code that relies on an implicit conversion to string.  Instead, we need to add \tcode{const char*} overloads to avoid ambiguity.}
...
namespace std {
  ...
  class basic_string_ref {
    ...
    // \ednote{\tcode{reverse_iterator} methods aren't \tcode{constexpr} because \tcode{reverse_iterator} isn't a literal type.  See LWG Issue 2208.}
...
\tcode{size()}      &   \tcode{traits::length(str)}   \\ \rowsep
\end{libefftabvalue}
...
\pnum
\requires
\tcode{str} is not a null pointer and \range{str}{str + len} is a valid range.
...
\pnum
\returns
A count of the number of char-like objects referred to by the \tcode{string_ref}.
...
\pnum
\enternote Unlike \tcode{basic_string::operator[]}, \tcode{basic_string_ref::operator[](size())}
has undefined behavior instead of returning \tcode{charT()}.
\exitnote
...
\begin{itemdescr}
\pnum
\returns A non-null pointer \tcode{p} such that \tcode{p + i == \&operator[](i)}
for each \tcode{i} in \range{0}{size()}.
...
\enterexample A sample conforming implementation for \tcode{operator==} would be:
\begin{codeblock}
...
namespace std {
  ...
  public:
    ...
    explicit basic_string(basic_string_ref<charT, traits>,
                          const Allocator& = Allocator());@\footnote{This conversion is explicit to avoid accidental \bigoh{N} operations on type mismatches.  It should be removed if a range constructor is added.}@
...
\rSec3[string.starts.ends.with]{basic_string::starts_with and ends_with}

\indexlibrary{\idxcode{basic_string}!\idxcode{starts_with}}%
\indexlibrary{\idxcode{starts_with}!\idxcode{basic_string}}%
\begin{itemdecl}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{basic_string_ref<charT, traits>(*this).starts_with(prefix)}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{basic_string_ref<charT, traits>(*this).ends_with(suffix)}
\end{itemdescr}
