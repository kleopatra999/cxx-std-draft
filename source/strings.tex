...
\end{codeblock}
is equivalent to \tcode{fx1(basic_string_ref(s))}.
...
\end{codeblock}
is equivalent to \tcode{fx2(basic_string_ref(\&c, 1))}.
...
namespace std {
  ...
  public:
    ...
    explicit@\footnote{This conversion is explicit to avoid accidental \bigoh{N} operations on type mismatches.}@ basic_string(basic_string_ref<charT, traits>,
...
\rSec3[string.starts.ends.with]{\tcode{basic_string::starts_with} and \tcode{ends_with}}
...
\end{codeblock}
is equivalent to \tcode{fn(string(str), idx, base)}.
...
\end{codeblock}
is equivalent to to \tcode{fn(string(str), idx)}.
...
\end{codeblock}
is equivalent to \tcode{fn(wstring(str), idx, base)}.
...
\end{codeblock}
is equivalent to \tcode{fn(wstring(str), idx)}.
