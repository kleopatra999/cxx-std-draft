...
namespace std {
  template<> struct char_traits<char> {
    ...
    static constexpr@\footnote{
        Implementing these functions efficiently at both compile and runtime requires implementation-specific magic that's not specified in this standard.
    }@ const char_type* find(const char_type* s, size_t n,
                 const char_type& a);
...
\pnum
\tcode{<string>} also defines the \tcode{basic_string_ref} class template for
referring to constant sequences of char-like objects and four
typedefs, \tcode{string_ref}, \tcode{u16string_ref}, \tcode{u32string_ref},
and \tcode{wstring_ref}, that name the
specializations \tcode{basic_string_ref<char>},
\tcode{basic_string_ref<char16_t>},
\tcode{basic_string_ref<char32_t>},
and
\tcode{basic_string_ref<wchar_t>}, respectively.
...
namespace std {
  ...
  // \ref{basic.string.ref}, basic_string_ref:
  template<class charT, class traits = char_traits<charT>>
      class basic_string_ref;

  // \ref{string.ref.comparison}, non-member basic_string_ref comparison functions
  ...
  // that has an implicit conversion to \tcode{basic_string_ref<charT, traits>}.

  // \ref{string.ref.nonmem}, other non-member basic_string_ref functions
  template<class charT, class traits = char_traits<charT>,
  ...
  // \tcode{basic_string_ref} typedef names
  ...
  // \ednote{If we remove the \tcode{string} overloads, it could break code that relies on an implicit conversion to string.  If we leave the \tcode{string} overloads, we need to also add \tcode{const char*} overloads since that conversion will now be ambiguous.}
  // \ednote{We'd like to add \tcode{optional<T> stox_consume(string_ref\&, int base=10)} if/when \tcode{optional} becomes available.}
...
\rSec1[basic.string.ref]{Class template \tcode{basic_string_ref}}

\pnum
\indexlibrary{\idxcode{basic_string_ref}}%
The class template \tcode{basic_string_ref} describes objects that can refer to
a constant sequence of arbitrary char-like objects with the first element of the
sequence at position zero.  In the rest of this Clause, the type of the
char-like objects held in a \tcode{basic_string_ref} object is designated
by \tcode{charT}.

\pnum
\enternote The library provides implicit conversions from \tcode{const charT*}
and \tcode{std::basic_string<charT, ...>} to \tcode{std::basic_string_ref<charT,
...>} so that user code can accept just \tcode{std::basic_string_ref<charT>} as
a parameter wherever a sequence of characters is expected.  User-defined types
should define their own implicit conversions to \tcode{std::basic_string_ref} in
order to interoperate with these functions. \exitnote

\pnum
The complexity of member functions is \bigoh{1} unless otherwise specified.
...
namespace std {
  ...
  class basic_string_ref {
    ...
    typedef @\impdef@ const_iterator; // See \ref{string.ref.iterators}
    typedef const_iterator iterator;@\footnote{Because \tcode{basic_string_ref} refers to a constant sequence, \tcode{iterator} and \tcode{const_iterator} are the same type.}@
    ...
    // \ref{string.ref.cons}, construct/copy
    ...
    // \ednote{No \tcode{initializer_list} constructor because [\ref{dcl.init.list}]p6 says it would likely store a dangling reference into the string_ref.}

    // \ref{string.ref.iterators}, iterators
    constexpr const_iterator begin() const noexcept;
    constexpr const_iterator end() const noexcept;
    constexpr const_iterator cbegin() const noexcept;
    constexpr const_iterator cend() const noexcept;
    // \ednote{\tcode{reverse_iterator} methods aren't \tcode{constexpr} because \tcode{reverse_iterator} isn't a literal type. It could only be a literal type on random-access base iterators, and I'm not sure we want that special case.}
    const_reverse_iterator rbegin() const noexcept;
    const_reverse_iterator rend() const noexcept;
    const_reverse_iterator crbegin() const noexcept;
    const_reverse_iterator crend() const noexcept;

    // \ref{string.ref.capacity}, capacity
    constexpr size_type size() const noexcept;
    constexpr size_type length() const noexcept;
    constexpr size_type max_size() const noexcept;
    constexpr bool empty() const noexcept;

    // \ref{string.ref.access}, element access
    ...
    // \ref{string.ref.modifiers}, modifiers:
    void clear() noexcept;
    void remove_prefix(size_type n);
    void remove_suffix(size_type n);

    // \ref{string.ref.ops}, string operations:
...
\end{codeblock}
requires that \range{s}{s+traits::length(s)} is a valid
range\footnote{Implementations are not required to handle arguments that are not
null terminated.} and returns \tcode{fx1(basic_string_ref(s))}.

\pnum
Each member function of the form
\begin{codeblock}
rt fx2(charT c);             // such as find()
\end{codeblock}
returns \tcode{fx2(basic_string_ref(\&c, 1))}.

\rSec2[string.ref.cons]{\tcode{basic_string_ref} constructors and assignment operators}

\indexlibrary{\idxcode{basic_string_ref}!constructor}%
\begin{itemdecl}
...
\pnum
\effects
Constructs an empty \tcode{basic_string_ref}.

\postcondition
\tcode{empty() == true} and \range{data()}{data()} is a valid range.
...
\pnum
\requires
\range{str}{str + traits::length(str)} must be a valid range.

\pnum
\effects
Constructs a \tcode{basic_string_ref} referring to the same string as \tcode{str}, with the postconditions in Table~\ref{tab:string.ref.ctr.1}

\begin{libefftabvalue}{\tcode{basic_string_ref(const charT*)} effects}{tab:string.ref.ctr.1}
\tcode{data()}      &   \tcode{str}                                                            \\ \rowsep
\tcode{size()}      &   0 if \tcode{str == nullptr} or \tcode{traits::length(str)} otherwise   \\ \rowsep
\end{libefftabvalue}
...
\pnum
\requires
\range{str}{str + len} must be a valid range.

\effects
Constructs a \tcode{basic_string_ref}, with the postconditions in Table~\ref{tab:string.ref.ctr.2}

\begin{libefftabvalue}{\tcode{basic_string_ref(const charT*, size_type)} effects}{tab:string.ref.ctr.2}
\tcode{data()}      &   \tcode{str}   \\ \rowsep
\tcode{size()}      &   \tcode{len}   \\ \rowsep
\end{libefftabvalue}
\end{itemdescr}

\rSec2[string.ref.iterators]{\tcode{basic_string_ref} iterator support}

\indexlibrary{\idxcode{const_iterator}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{const_iterator}}%
\begin{itemdecl}
...
\pnum
For a \tcode{basic_string_ref} \tcode{str}, any operation that
invalidates a pointer in the range
\range{str.data()}{str.data()+str.size()} invalidates pointers and
iterators returned from \tcode{str}'s methods.
\end{itemdescr}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{begin}}%
\indexlibrary{\idxcode{begin}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{cbegin}}%
\indexlibrary{\idxcode{cbegin}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
constexpr const_iterator begin() const noexcept;
constexpr const_iterator cbegin() const noexcept;
\end{itemdecl}
...
\indexlibrary{\idxcode{end}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{end}}%
\indexlibrary{\idxcode{cend}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{cend}}%
\begin{itemdecl}
constexpr const_iterator end() const noexcept;
constexpr const_iterator cend() const noexcept;
\end{itemdecl}
...
\indexlibrary{\idxcode{rbegin}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{rbegin}}%
\indexlibrary{\idxcode{crbegin}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{crbegin}}%
\begin{itemdecl}
const_reverse_iterator rbegin() const noexcept;
const_reverse_iterator crbegin() const noexcept;
\end{itemdecl}
...
\indexlibrary{\idxcode{rend}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{rend}}%
\indexlibrary{\idxcode{crend}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{crend}}%
\begin{itemdecl}
const_reverse_iterator rend() const noexcept;
const_reverse_iterator crend() const noexcept;
\end{itemdecl}
...
\rSec2[string.ref.capacity]{\tcode{basic_string_ref} capacity}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{size}}%
\indexlibrary{\idxcode{size}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\pnum
\returns
A count of the number of char-like objects referred to by the string_ref.

\pnum
\complexity constant time.
\end{itemdescr}

\indexlibrary{\idxcode{length}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{length}}%
\begin{itemdecl}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{max_size}}%
\indexlibrary{\idxcode{max_size}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\pnum
\returns
The size of the largest possible string_ref.
\end{itemdescr}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{empty}}%
\indexlibrary{\idxcode{empty}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\rSec2[string.ref.access]{\tcode{basic_string_ref} element access}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{operator[]}}%
\indexlibrary{\idxcode{operator[]}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\pnum
\returns \tcode{*(begin() + pos)}

\pnum
\throws Nothing.
\end{itemdescr}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{at}}%
\indexlibrary{\idxcode{at}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{front}}%
\indexlibrary{\idxcode{front}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
constexpr const charT& front() const;
\end{itemdecl}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{back}}%
\indexlibrary{\idxcode{back}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
constexpr const charT& back() const;
\end{itemdecl}
...
\indexlibrary{\idxcode{data}!\tcode{basic_string_ref}}%
\begin{itemdecl}
...
\tcode{i} in \range{0}{size()}.

\pnum
\enternote Unlike \tcode{std::string::data()} and string literals,
\tcode{data()} may return a pointer to a buffer that is not null-terminated.
Therefore it is typically a mistake to pass \tcode{data()} to a routine that
takes just a \tcode{const charT*} and expects a null-terminated string.
\exitnote
\end{itemdescr}

\rSec2[string.ref.modifiers]{\tcode{basic_string_ref} modifiers}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{clear}}%
\indexlibrary{\idxcode{clear}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
void clear() noexcept;
\end{itemdecl}
...
\pnum
\effects
Same as \tcode{*this = basic_string_ref()}
\end{itemdescr}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{remove_prefix}}%
\indexlibrary{\idxcode{remove_prefix}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\pnum
\effects
Same as \tcode{*this = substr(n, npos)}
\end{itemdescr}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{remove_suffix}}%
\indexlibrary{\idxcode{remove_suffix}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\pnum
\effects
Same as \tcode{*this = substr(0, size() - n)}
\end{itemdescr}

\rSec2[string.ref.ops]{\tcode{basic_string_ref} string operations}

\pnum
\enternote Unlike \tcode{std::basic_string}, \tcode{std::basic_string_ref}
provides no whole-string methods with position or length parameters. Instead,
users should use the \tcode{substr()} method to create the character sequence
they're actually interested in, and use that.  \exitnote

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{substr}}%
\indexlibrary{\idxcode{substr}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\pnum
\effects
Determines the effective length \tcode{rlen} of the string to reference as the
smaller of \tcode{n} and \tcode{size() - pos}.

\pnum
\returns
\tcode{basic_string_ref(data()+pos, rlen)}.
\end{itemdescr}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{compare}}%
\indexlibrary{\idxcode{compare}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\pnum
\complexity \bigoh{rlen}

\pnum
\returns
The nonzero result if the result of the comparison is nonzero.
Otherwise, returns a value as indicated in Table~\ref{tab:string.ref.compare}.

\begin{floattable}{\tcode{compare()} results}{tab:string.ref.compare}
{lc}
\topline
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{starts_with}}%
\indexlibrary{\idxcode{starts_with}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\begin{itemdescr}
\pnum
\complexity \bigoh{\tcode{min(size(), prefix.size())}}

\returns
\tcode{true} if \tcode{prefix} is a prefix of \tcode{*this}.
\end{itemdescr}

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{ends_with}}%
\indexlibrary{\idxcode{ends_with}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\begin{itemdescr}
\pnum
\complexity \bigoh{\tcode{min(size(), suffix.size())}}

\returns
\tcode{true} if \tcode{suffix} is a suffix of \tcode{*this}.
\end{itemdescr}

\rSec3[string.ref.find]{Searching \tcode{basic_string_ref}}

\pnum
Member functions in this section have complexity \bigoh{\tcode{size() *
\textit{argument}.size()}} at worst, although implementations are encouraged
to do better.

\indexlibrary{\idxcode{basic_string_ref}!\idxcode{find}}%
\indexlibrary{\idxcode{find}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
for all elements \tcode{I} of the string referenced by \tcode{str}.
\end{itemize}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{rfind}}%
\indexlibrary{\idxcode{rfind}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{find_first_of}}%
\indexlibrary{\idxcode{find_first_of}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
for some element \tcode{I} of the string referenced by \tcode{str}.
\end{itemize}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{find_last_of}}%
\indexlibrary{\idxcode{find_last_of}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{find_first_not_of}}%
\indexlibrary{\idxcode{find_first_not_of}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\indexlibrary{\idxcode{eq}!\tcode{char_traits}}%
\tcode{traits::eq(at(xpos), str.at(I))}
for no element \tcode{I} of the string referenced by \tcode{str}.
\end{itemize}
...
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{find_last_not_of}}%
\indexlibrary{\idxcode{find_last_not_of}!\idxcode{basic_string_ref}}%
\begin{itemdecl}
...
\rSec2[string.ref.comparison]{\tcode{basic_string_ref} non-member comparison functions}

\pnum
Implementations shall provide sufficient additional overloads so that an object
\tcode{t} with an implicit conversion to \tcode{basic_string_ref<charT, traits>}
can be compared according to Table~\ref{tab:string.ref.comparison.overloads},
where \tcode{sp} is an instance of \tcode{basic_string_ref<charT,
traits>}.\footnote{Implementations could accomplish this by defining
\begin{codeblock}^^J
template<typename T> struct __identity \{ typedef T type; \};^^J
template<typename charT, typename traits>^^J
bool operator==(basic_string_ref<charT, traits> lhs,^^J
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ basic_string_ref<charT, traits> rhs) \{^^J
\ \ return lhs.compare(rhs) == 0;^^J
\}^^J
template<typename charT, typename traits>^^J
bool operator==(basic_string_ref<charT, traits> lhs,^^J
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ typename __identity<basic_string_ref<charT, traits>>::type rhs) \{^^J
\ \ return lhs.compare(rhs) == 0;^^J
\}^^J
template<typename charT, typename traits>^^J
bool operator==(typename __identity<basic_string_ref<charT, traits>>::type lhs,^^J
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ basic_string_ref<charT, traits> rhs) \{^^J
\ \ return lhs.compare(rhs) == 0;^^J
\}^^J
\end{codeblock}
...
\begin{libtab2}{Additional \tcode{basic_string_ref} comparison overloads}
{tab:string.ref.comparison.overloads}{ll}
{Expression}{Returns}
\tcode{t \ == sp}  &  \tcode{basic_string_ref<charT, traits>(t) == sp} \\ \rowsep
\tcode{sp == t}  &  \tcode{sp == basic_string_ref<charT, traits>(t)} \\ \rowsep
\tcode{t \ != sp}  &  \tcode{basic_string_ref<charT, traits>(t) != sp} \\ \rowsep
\tcode{sp != t}  &  \tcode{sp != basic_string_ref<charT, traits>(t)} \\ \rowsep
\tcode{t \ < \ sp}  &  \tcode{basic_string_ref<charT, traits>(t) < sp} \\ \rowsep
\tcode{sp < \ t}  &  \tcode{sp < basic_string_ref<charT, traits>(t)} \\ \rowsep
...
\tcode{sp <= t}  &  \tcode{sp <= basic_string_ref<charT, traits>(t)} \\ \rowsep
\tcode{t \ >= sp}  &  \tcode{basic_string_ref<charT, traits>(t) >= sp} \\ \rowsep
\tcode{sp >= t}  &  \tcode{sp >= basic_string_ref<charT, traits>(t)} \\
...
\rSec2[string.ref.nonmem]{Other \tcode{basic_string_ref} non-member functions}

\indexlibrary{\idxcode{to_string}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{to_string}}%
\begin{itemdecl}
template<class charT, class traits = char_traits<charT>,
         class Allocator = allocator<charT> >
  basic_string<charT, traits, Allocator> to_string(
...
\begin{itemdescr}
\pnum
\complexity \bigoh{\tcode{str.size()}}

\pnum
\returns
\tcode{basic_string<charT, traits, Allocator>(str, a)}.
\end{itemdescr}
...
namespace std {
  ...
  public:
    ...
    explicit basic_string(basic_string_ref<charT, traits>,
                          const Allocator& = Allocator());@\footnote{This conversion is explicit to avoid accidental \bigoh{N} operations on type mismatches.}@
   ...
   ~basic_string();
    ...
    iterator insert(const_iterator p, basic_string_ref<charT, traits>);
...
\indexlibrary{\idxcode{basic_string}!constructor}%
\begin{itemdecl}
explicit basic_string(basic_string_ref<charT, traits> str, const Allocator& a = Allocator());\footnote{
This constructor should be removed if a range constructor is added.}
\end{itemdecl}

\begin{itemdescr}
\pnum
\effects Same as \tcode{basic_string(str.begin(), str.end(), a)}.
\end{itemdescr}
...
\indexlibrary{\idxcode{operator basic_string_ref}!\tcode{basic_string}}%
\indexlibrary{\idxcode{basic_string}!\tcode{operator basic_string_ref}}%
\begin{itemdecl}
operator basic_string_ref<charT, traits>() const noexcept;
\end{itemdecl}
...
\begin{itemdescr}
\pnum
\returns \tcode{basic_string_ref<charT, traits>(data(), size())}.
...
\rSec1[string.io]{Inserters and extractors}
...
\indexlibrary{\idxcode{operator\shl}!\idxcode{basic_string_ref}}%
\indexlibrary{\idxcode{basic_string_ref}!\idxcode{operator\shl}}%
\begin{itemdecl}
template<class charT, class traits, class Allocator>
  basic_ostream<charT, traits>&
    operator<<(basic_ostream<charT, traits>& os,
...
\end{codeblock}
acts as if it were a call to \tcode{fn(string(str), idx, base)}.
...
\end{codeblock}
acts as if it were a call to \tcode{fn(string(str), idx)}.
...
\end{codeblock}
acts as if it were a call to \tcode{fn(wstring(str), idx, base)}.
...
\end{codeblock}
acts as if it were a call to \tcode{fn(wstring(str), idx)}.
