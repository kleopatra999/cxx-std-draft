...
\begin{itemize}
\item
Two member operator templates
\tcode{operator()(const basic_string<C, T, A>\&, const basic_string<\brk{}C, T, A>\&)}
and
\tcode{operator()(const basic_string_ref<C, T>\&, const basic_string_ref<\brk{}C, T>\&)}
are provided so that a locale may be used as a predicate argument to
...
sequence beginning at \tcode{ptr} to a wide string. The third and fourth member functions
shall convert the sequence stored in \tcode{str} to a wide string. The fifth member
...
a byte string. The third and fourth member functions shall convert the sequence stored in \tcode{wstr} to a
byte string. The fifth member function shall convert the sequence defined by the
